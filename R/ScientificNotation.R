...
#' @describeIn ScientificNotation Gets the SI prefix for a number
...
exponent_to_si <- function(exp) {
  gsub("[^a-zA-Z]", "", sitools::f2si(10 ^ exp))
}
...
get_exp_eng <- function(val) {
  ...
  decimals[which.min(abs(decimals - stats::median(as.numeric(
    stringr::str_sub(format(val, scientific = T),
                     (
                       stringr::str_locate(format(val, scientific = T),
                                           "e[\\+\\-]")
                     )
                     [, 2])
  ))))]
